...
const ConsoleLogDetails: React.FC<ConsoleLogDetailsProps> = ({ entry, onClose }) => {
  ...
  const safeFormatDate = (timestamp: string | Date): string => {
  ...
  const copyFullEntry = () => {
    const fullText = `[${entry.level.toUpperCase()}] ${safeFormatDate(entry.timestamp)}
        ...
        {activeTab === 'overview' && (
          <div className="details-section">
            ...
            <div className="detail-row">
              ...
              <span className="detail-value">{safeFormatDate(entry.timestamp)}</span>
            </div>
