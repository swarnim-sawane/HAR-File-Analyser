...
describe('HAR entry storage preparation', () => {
  it('preserves normal HAR entries for MongoDB storage', () => {
  ...
  it('truncates oversized response and request body text before MongoDB storage', () => {
    ...
    expect(stored.response.content.text).toContain('[truncated for MongoDB storage');
    expect(stored.request.postData.text).toContain('[truncated for MongoDB storage');
