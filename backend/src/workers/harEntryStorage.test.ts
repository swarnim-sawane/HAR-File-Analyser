...
describe('HAR entry storage preparation', () => {
  it('preserves normal HAR entries for analyzer storage', () => {
  ...
  it('truncates oversized response and request body text before analyzer storage', () => {
    ...
    expect(stored.response.content.text).toContain('[truncated for analyzer storage');
    expect(stored.request.postData.text).toContain('[truncated for analyzer storage');
