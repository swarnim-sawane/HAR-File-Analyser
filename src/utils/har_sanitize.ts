...
function redactDomainsInHar(input: string, domains: string[]): string {
    ...
    const replaceAll = (s: string): string => {
        for (const domain of domains) {
            // Use a literal-string replaceAll so dots in the domain don't act as regex wildcards
    ...
    for (const entry of entries) {
        ...
        // Request headers (Host, Origin, Referer, etc.)
        ...
        // Response headers (Location, Access-Control-Allow-Origin, etc.)
    ...
    // Also sanitize page titles which often contain the site URL
