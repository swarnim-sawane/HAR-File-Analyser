...
The project is suitable for handoff as an internal diagnostic application. The main architecture is understandable and maintainable: React frontend, Express backend API, BullMQ worker, Oracle JSON persistence, Redis-compatible queue/pub-sub, optional Qdrant, and optional OCA-backed AI.
...
- Redis-compatible cache: appropriate for BullMQ queues, upload progress, and pub/sub.
...
| P0 | OCI persistence validation | Current migration branch uses Oracle JSON persistence and a Redis-compatible cache interface. | Validate Oracle Database JSON storage and OCI cache connectivity with representative HAR/log volumes before org-wide rollout. |
...
2. Confirm the Oracle Database JSON environment, cache endpoint, and ownership model for the development team.
