...
The project is suitable for handoff as an internal diagnostic application. The main architecture is understandable and maintainable: React frontend, Express backend API, Oracle-backed worker queue/runtime state, Oracle JSON persistence, optional Qdrant, and optional OCA-backed AI.
...
- Oracle runtime documents: appropriate for this experimental branch's queue jobs, upload progress, and event delivery while the owning team validates performance and operational limits.
...
| P0 | OCI persistence validation | Current migration branch uses Oracle Database for analyzer persistence plus runtime cache, queueing, and event delivery. | Validate Oracle Database JSON storage, queue polling, event polling, and cleanup behavior with representative HAR/log volumes before org-wide rollout. |
